...
**Multiple models with `model-policy`** *(v1.0.83+)*: `model` can list several models, tried in order until one is available to your account:
...
Setting `model-policy: required` keeps the agent pinned to models from that list even if the user tries to switch to a different model mid-session—useful when an agent's instructions are tuned for a specific model family.
