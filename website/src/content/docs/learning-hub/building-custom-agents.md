...
**Model fallback lists** *(v1.0.83+)*: `model` can also be a list of several models, tried in order until one is available to you — useful when an agent's preferred model is rate-limited, region-restricted, or not enabled on your account:
...
Add `model-policy: required` to keep the session locked to models on that list even if the user tries to switch away with `/model`:
