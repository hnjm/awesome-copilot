...
**Model fallback lists (v1.0.83+)**: `model` can also list several models, tried in order until one is available to you—useful when a preferred model is rate-limited, disabled by policy, or not yet rolled out to your plan:
...
Add `model-policy: required` to keep model changes restricted to that list — end users can still switch between the listed models but cannot pick a model outside it.
