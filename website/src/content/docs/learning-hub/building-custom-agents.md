...
> **Model fallback lists (v1.0.83+)**: `model` can list several models instead of just one — the CLI tries each in order until it finds one you have access to. Combine this with `model-policy: required` to keep the agent restricted to that list instead of silently falling back to your session's default model when none of the listed models are available:
>
> ```yaml
> ---
> name: 'Security Reviewer'
