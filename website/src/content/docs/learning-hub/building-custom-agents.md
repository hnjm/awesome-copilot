...
> **Model fallback lists** *(v1.0.83+)*: `model` can also be an array of models, tried in order until one is available to you — useful when your preferred model is temporarily unavailable or not enabled for your account:
>
> ```yaml
> model: [Claude Opus 5, Claude Sonnet 4]
> ```
>
> Add `model-policy: required` to keep the agent restricted to models on that list, preventing a user from switching to an unlisted model mid-session.
