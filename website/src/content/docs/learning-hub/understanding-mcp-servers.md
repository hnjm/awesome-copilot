...
- **Client ID Metadata Document (CIMD) support** *(v1.0.83+)*: MCP OAuth sign-in now supports CIMD, letting servers reference client metadata by URL instead of requiring pre-registered client IDs.
- **Windows authentication broker (WAM)** *(v1.0.81+)*: On Windows, remote MCP servers protected by Microsoft Entra ID can sign in through the OS authentication broker, usually with no prompt at all. Other platforms, `--device-code`, and machines without the broker library keep the existing browser flow.
