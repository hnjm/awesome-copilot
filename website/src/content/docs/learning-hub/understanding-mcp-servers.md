...
- **Microsoft Entra ID (Azure AD)**: MCP servers that authenticate via Microsoft Entra ID are fully supported. Once you complete the initial login, the CLI caches the authentication and **will not show the consent screen on subsequent connections** — you authenticate once per session rather than every time the server reconnects. On Windows *(v1.0.81+)*, sign-in for Entra ID-protected remote MCP servers can go through the OS authentication broker (WAM), typically with no prompt at all; other platforms, `--device-code`, and machines without the broker library fall back to the existing browser flow.
- **Client ID Metadata Documents (CIMD)** *(v1.0.83+)*: MCP OAuth sign-in supports CIMD, letting an MCP server verify the CLI's client identity from a hosted metadata document instead of requiring pre-registered client credentials — useful for enterprise servers with stricter OAuth client validation.
